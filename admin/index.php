			...
			<table class="table mt-1 mb-1 bg-light">
				<!-- début tableau affichage des articles -->
					...
					<?php foreach($affiche_categorie as $ligne_articles):?><!-- on affiche toutes les données de la table articles -->
						<tr class="bg-dark">		
							<td class="text-center text-light"><?= $ligne_articles['id_work']; ?></td>
							<td class="text-center text-light"><?= $ligne_articles['id_category']; ?></td>
							<td class="text-center text-light"><?= $ligne_articles['category_name']; ?></td>
							<td class="text-center text-light"><?= $ligne_articles['work_name']; ?></td>
							<td class="text-center text-light"><?= $ligne_articles['work_url']; ?></td>
							<!-- Le "?" dans le href ci-dessous permet de récupérer 'supprimer' avec la méthode GET-->
							<td><a href="article_edit.php?modifier=<?= $ligne_articles['id_work']; ?>" class="btn btn-info btn-block">Modifier</a></td>
